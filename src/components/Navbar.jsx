    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            ...
            <li key={link.href}>
              <a
                ...
                className={`text-sm font-medium transition-colors hover:text-blue-400 ${
                  active === link.href.slice(1) ? 'text-blue-400' : 'text-gray-700 dark:text-gray-300'
                }`}
              >
                {link.label}
              </a>
        ...
        <div className="flex items-center gap-3 md:hidden">
          ...
          <button onClick={() => setMenuOpen(!menuOpen)} className="text-gray-700 dark:text-gray-300 hover:text-blue-500 dark:hover:text-white">
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="md:hidden bg-white/95 dark:bg-gray-900/95 backdrop-blur-md px-4 pb-4">
          {navLinks.map(link => (
            <a
              key={link.href}
              href={link.href}
